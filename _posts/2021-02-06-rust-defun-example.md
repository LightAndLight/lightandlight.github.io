...
fn map_freevars<F: Fn(usize) -> usize>(f: F, e: &Expr) -> Self {
    match e {
...
```rust
fn map_freevars(f: &dyn Fn(usize) -> usize, e: &Expr) -> Self {
    match e {
        Expr::Var(n) => Expr::Var(f(*n)),
        Expr::App(a, b) => Expr::App(Box::new(map_freevars(f, a)), Box::new(map_freevars(f, b))),
        Expr::Lam(b) => Expr::Lam(Box::new(map_freevars(
...
```rust
fn map_freevars<'a, F: Fn(usize) -> usize>(f: &'a Origin<'a, F>, e: &Expr) -> Self {
    match e {
